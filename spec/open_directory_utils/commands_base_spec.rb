  ...
  context "test dseditgroup" do
    describe "dseditgroup build by hand" do
      it "all good data - check user membership" do
        ...
        correct = '/usr/sbin/dseditgroup -o checkmember -u diradmin -P "TopSecret" -n /LDAPv3/127.0.0.1 -m username groupname'
        expect( answer ).to eq( correct )
      end
      ...
      it "all good data - to add a user" do
        attribs = { operation: 'edit', value: 'groupname',
                    record_name: 'username', action: 'add', type: 'user'}
        answer  = base.send(:dseditgroup, attribs, srv_info)
        correct = '/usr/sbin/dseditgroup -o edit -u diradmin -P "TopSecret" -n /LDAPv3/127.0.0.1 -a username -t user groupname'
        expect( answer ).to eq( correct )
      end
      it "all good data - to remove a user" do
        ...
        answer  = base.send(:dseditgroup, attribs, srv_info)
        correct = '/usr/sbin/dseditgroup -o edit -u diradmin -P "TopSecret" -n /LDAPv3/127.0.0.1 -d username -t user groupname'
        expect( answer ).to eq( correct )
      end
      it "all good data - create a group" do
        attribs = { operation: 'create', value: 'groupname', real_name: 'Group Name'}
        answer  = base.send(:dseditgroup, attribs, srv_info)
        correct = '/usr/sbin/dseditgroup -o create -u diradmin -P "TopSecret" -n /LDAPv3/127.0.0.1 -r "Group Name" groupname'
        expect( answer ).to eq( correct )
      end
      it "all good data - create a group - no realname" do
        attribs = { operation: 'create', value: 'groupname'}
        answer  = base.send(:dseditgroup, attribs, srv_info)
        correct = '/usr/sbin/dseditgroup -o create -u diradmin -P "TopSecret" -n /LDAPv3/127.0.0.1 -r "groupname" groupname'
        expect( answer ).to eq( correct )
      end
      it "all good data - delete a group" do
        attribs = { operation: 'delete', value: 'groupname'}
        answer  = base.send(:dseditgroup, attribs, srv_info)
        correct = '/usr/sbin/dseditgroup -o delete -u diradmin -P "TopSecret" -n /LDAPv3/127.0.0.1 groupname'
        expect( answer ).to eq( correct )
      end
    end
